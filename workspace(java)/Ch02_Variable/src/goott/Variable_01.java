 ...
 *   값을 읽어 오는 것.
 * - 데이터 값을 저장하려면 메모리에 값을 저장할 공간을 생성하고, 이름을 부여해야함.
 ...
 *   이를 자료형이라 함. 즉, 메모리 공간의 자료형에 따라 저장할 수 있는 값의 종류와 크기가 결정됨.
 *   
 * 변수 선언하는 방법
 * 형식) 자료형 변수명;
 ...
 * 4. 대소문자는 구분함. 예) SUM과 sum은 다른 변수명으로 인식함.
 * 5. 변수명은 의미있게 만드는 것이 좋음.
 * 6. 변수명에 두 개의 이상의 단어가 결합이 된 경우에는 
 *    다음 글자의 첫글자는 대문자로 작성하는 것이 관례임.(CamelCase 기법)
 *    
 */
