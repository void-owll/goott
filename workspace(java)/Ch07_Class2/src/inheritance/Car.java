 ...
 *   하위 객체가 상위 객체의 속성을 모두 갖는 개념임.
 * - 자바에서의 상속은 자식클래스가 부모클래스의 속성을 물려 받고
 *   기능을 추가하여 확장(extends)하는 개념임.
 * - 상속은 슈퍼클래스의 필드와 메서드를 물려 받아서 코드를 재사용함으로써,
 *   코드 작성에 드는 시간과 비용을 줄일 수 있음.
 * - 기존에 이미 잘 만들어진 클래스를 재사용하여 새로운 클래스를 생성하는 자바 문법. **중요함**
 *   즉, 특정 클래스를 만들 때 기존의 클래스의 데이터(속성)와 기능(메서드)을 그대로 물려 받아서
 *   중복적인 코드를 없애줌. (클래스의 재사용, 코드의 중복을 없애줌)
